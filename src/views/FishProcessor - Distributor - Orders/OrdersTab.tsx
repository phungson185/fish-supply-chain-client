...
const OrdersTab = ({ status }: { status: ProcessStatus }) => {
  ...
  const [dataSearch, onSearchChange] = useSearch({
    page,
    receiver: role === RoleType.fishProcessorRole ? id : undefined,
    orderer: role === RoleType.distributorRole ? id : undefined,
        ...
        {items.map((item) => (
          ...
          >
            <div className='flex flex-row items-center gap-2 border-b-2 border-solid border-gray-200 pb-3 mb-3'>
              <Avatar
                src={role === RoleType.fishProcessorRole ? item.orderer.avatar ?? '' : item.receiver.avatar ?? ''}
              />
              <div className='font-bold'>
                {role === RoleType.fishProcessorRole ? item.orderer.name ?? '' : item.receiver.name ?? ''}
              </div>
            ...
            <div className='flex flex-row justify-between items-center'>
              <Typography variant='caption' className='block'>
                Transaction hash: {item.processedFishPurchaseOrderId}
              </Typography>
              <Typography variant='caption' className='block'>
